...
export const onFeedUpdatedConsumer = async (context: {
  ...
  const routingKey = 'feedUpdated'
  const queueName = 'feedUpdated_queue'
  ...
  await ch.consume(queueName, async function (message) {
    LOGGER.debug('[@realtime/amqp-consumer]: on-feed-updated')
    ...
    LOGGER.info(`[@realtime/amqp-consumer]: on-feed-updated to ${JSON.stringify(JSON.parse(message.content.toString()))}`)
  })
